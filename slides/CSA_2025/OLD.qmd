...
```{r esempio1, warning=FALSE,message=FALSE}
library(overlapping)
library(dplyr)
...
<!-- The estimated overlapping area is $\eta =$ `r round(OV$OV,2)`  -->

## The Overlapping Index

The overlapping ($\eta$) is an index of effect size and varies from 0 (no overlap between to densities of the data) and 1 (complete overlap), formally defined in the following way:
...
-   rearrange the data $B$ times
-   calculate the statistic for each new dataset of permuted data
-   calculate $p$- value as the probability of obtaining equal or more extreme values
...
<!-- Now instead of working with $\eta$, it is more intuitive to work with the complement $1 - \eta = \zeta$ and define $H_0 : \zeta = 0$ meaning that there is no difference between the densities of the two populations. -->
...
```{r permtest, results='markup',include=TRUE}
OV<-OV$OV[1]
OV
OV_P
...
![](img/global_1.JPG){fig-align="center"}

## Discussion

-The $\zeta$ overlapping test is the most powerful choice, especially with small samples. - It is an intuitive index and it allows to investigate group differences behiond the mean - It also pushes the focus to a more nuanced approach to testing
...
```{r, warning=FALSE,echo=FALSE,message=FALSE, fig.width=5, include=FALSE}

library(overlapping)
library(ggplot2)

EngTurk <- read.csv("~/Library/CloudStorage/OneDrive-UniversitàdegliStudidiPadova/Lavoro/Conferences/Presentazioni/CSA_2025/data/EngTurk.csv")
...
<!-- ## Who of you works with groups or conditions? -->

<!-- - control group and treatment group  -->

<!-- - athletes vs. non-athletes -->

<!-- - frequent stimuli vs. less frequent stimuli (i.e. words) -->
...
<!-- Were your data always normally distributed? -->

<!-- ## {} -->

<!-- Here a very common example: -->
...
<!-- - Was I supposed to check the data distribution?? -->

<!-- - Used a $t$-test -->

<!-- - Transformed the variable -->

<!-- - Used test on ranks (i.e. Wilcoxon-Mann-Whitney) -->
