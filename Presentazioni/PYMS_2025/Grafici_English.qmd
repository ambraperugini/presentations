...
plot_likert_molto <- function(colonna, titolo) {
  ...
  mappa_likert <- c(
    ...
    "Poco" = "A little", "poco" = "A little",
    "Abbastanza" = "Quite a bit", "abbastanza" = "Quite a bit",
  ...
  livelli_ordinati <- c("Not at all", "A little", "Quite a bit", "Very much")
...
plot_likert_completamente <- function(colonna, titolo) {
  ...
  livelli_ordinati <- c("Not at all", "A little", "Quite a bit", "Completely")
  dati_fattore <- factor(dati_raggruppati, levels = livelli_ordinati, ordered = TRUE)
...
datacleaned$Gruppo <- ifelse(datacleaned$"1" %in% c("PO, PA, RTD, RTT"), "Tenure track",
                             ifelse(datacleaned$"1" %in% c("Dottorato, assegno di ricerca"), "ECR's", NA))
...
plot_tasso_risposta <- function(totali_strutturati, totali_non_strutturati, titolo = "") {
  ...
  risposte_non_strutturati <- sum(datacleaned$Gruppo == "ECR's", na.rm = TRUE)
  
  # Creazione del dataframe per il grafico
  df <- data.frame(
    Gruppo = c("Tenure track", "ECR's"),
  ...
  ggplot(df, aes(x = Gruppo, y = Percentuale, fill = Gruppo)) +
    ...
    labs(
      title = titolo,
      subtitle = paste("Total: Tenure track =", totali_strutturati, ", ECR's =", totali_non_strutturati),
      x = "Group",
...
# Metodological Review Board (MRB)
...
**Establish a scalable and sustainable MRB by**:

- Supporting clarity and transparency in preregistration

- Offering advisory methodological feedback

- Maintaining impartiality (no co-authorship)
...
- Promoting a culture of preregistration, transparency, open science and the use
of better research practices

# The Survey

## Departmental Insights on:
...
Total respondents: 53, representing approximately (`r round(53/(65+77)*100,0)`\% of the department’s academic staff.

- Tenure-track: 31, `r round(31/53*100,0)`\% 

- Early-career researchers: 22, `r round(22/53*100,0)`\% 


## Do you have an OSF account? {.unlisted}
...
## Have you ever used of those practices? {.unlisted}
...
## Will you use one of those practices in your next stuy? {.unlisted}

```{r os10.1a}
plot_likert_molto("10.1", "Pre-registration of Hypotheses")
```

## Will you use one of those practices in your next stuy? {.unlisted}

```{r os10.1b}
plot_likert_gruppi_molto("10.1", "Pre-registration of Hypotheses")
```

## Will you use one of those practices in your next stuy? {.unlisted}

```{r os10.2a}
plot_likert_molto("10.2", "Pre-registration of Analyses")
```

## Will you use one of those practices in your next stuy? {.unlisted}

```{r os10.2b}
plot_likert_gruppi_molto("10.2", "Pre-registration of Analyses")
```
...
## For the statistical part, who do you usually consult? {.unlisted}

```{r mrb11.1a}
plot_frequenze("11.1", "Someone inside the research group")
```

## For the statistical part, who do you usually consult? {.unlisted}

```{r mrb11.1b}
plot_confronto_gruppi("11.1", "Someone inside the research group")
```

## For the statistical part, who do you usually consult? {.unlisted}

```{r mrb11.2a}
plot_frequenze("11.2", "External expert - Co-author")
```

## For the statistical part, who do you usually consult? {.unlisted}

```{r mrb11.2b}
plot_confronto_gruppi("11.2", "External expert - Co-author")
```

## For the statistical part, who do you usually consult? {.unlisted}

```{r mrb11.3a}
plot_frequenze("11.3", "External expert - NO Co-author")
```

## For the statistical part, who do you usually consult? {.unlisted}

```{r mrb11.3b}
plot_confronto_gruppi("11.3", "External expert - NO Co-author")
```
...
```{r mrb13.1a}
plot_likert_completamente("13.1", "o be a methodological point of reference")
```

## How much would a MRB be useful for: {.unlisted}

```{r mrb13.1b}
plot_likert_gruppi_completamente("13.1", "o be a methodological point of reference")
```
...
## Should the MRB be integrated in the Ethical Committee? {.unlisted}
